...
import { Car, Phone, ArrowRight, HelpCircle, X, ShieldCheck, Clock, Sparkles, AlertCircle, Loader2 } from 'lucide-react';
...
const TrackVehicle = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const plateInput = vehicleNumber.trim();
    const phoneInput = phoneNumber.trim();

    if (!plateInput) {
      setErrorMessage('Please enter your vehicle plate number or tracking token.');
      return;
    }
    ...
    try {
      // Determine if token or plate+phone lookup
      const isToken = plateInput.startsWith('tr_');
      const params = isToken
        ? { token: plateInput }
        : { plate: plateInput.toUpperCase(), phone: phoneInput };

      const res = await trackingService.trackVehicle(params);

      if (res.success && res.data) {
        const queryParams = new URLSearchParams();
        if (res.data.trackingToken) queryParams.set('token', res.data.trackingToken);
        if (res.data.vehiclePlate) queryParams.set('veh', res.data.vehiclePlate);
        if (phoneInput) queryParams.set('phone', phoneInput);

        navigate(`/track/details?${queryParams.toString()}`);
      }
    } catch (err) {
      setErrorMessage(
        err.message || 'No active vehicle service found matching the provided details. Please verify your vehicle registration or phone number.'
      );
    } finally {
      setSearching(false);
    }
              ...
              <form onSubmit={handleSubmit} className="space-y-4 pt-2">
                ...
                <div className="space-y-1.5 text-left">
                  <label htmlFor="vehicleNumber" className="block text-xs sm:text-sm font-semibold text-gray-800">
                    Vehicle Number or Tracking Token
                  </label>
                  <div className="relative flex items-center">
                    ...
                    <input
                      ...
                      placeholder="e.g. KA-01-MJ-8899 or tr_1a2b3c"
                ...
                <button
                  type="submit"
                  disabled={searching}
                  className="w-full bg-black hover:bg-gray-800 text-white font-semibold rounded-2xl py-4 px-4 flex items-center justify-center gap-2 text-base transition-all shadow-sm active:scale-[0.99] cursor-pointer mt-2 disabled:opacity-50"
                >
                  {searching ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" />
                      <span>Searching Vehicle...</span>
                    </>
                  ) : (
                    <>
